...
Implementacion pura basada en especificaciones DIAN Anexo Tecnico v1.9.
"""
...
from .config import (
    NS, C14N_ALG, RSA_SHA256, SHA256_ALG,
    ENVELOPED_SIG, SIGNED_PROPS_TYPE,
    POLITICA_URL, POLITICA_HASH
)
...
class XAdESSigner:
    ...
    def sign(self, xml_element: etree._Element, ext_ns: str = None) -> etree._Element:
        ...
        IMPORTANTE: El digest del documento se calcula ANTES de insertar la firma.
        ...
        cert_b64 = cert_to_base64(self.certificate)
        ...
        # =====================================================================
        doc_c14n = etree.tostring(xml_element, method='c14n', exclusive=False, with_comments=False)
        doc_digest = sha256_digest(doc_c14n)

        # =====================================================================
        # PASO 2: INSERTAR ESTRUCTURA DE FIRMA EN EL DOCUMENTO
        # =====================================================================
        ext_contents = xml_element.findall('.//{%s}ExtensionContent' % ext_ns)
        ...
        if self.chain:
            etree.SubElement(x509d, '{%s}X509Certificate' % NS['ds']).text = cert_to_base64(self.chain[0])
        ...
        add_cert(self.certificate, sc)
        if self.chain:
            add_cert(self.chain[0], sc)
